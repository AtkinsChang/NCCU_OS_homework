...
import edu.nccu.plsm.osproject.queue.Range;
...
/**
 * Modified {@link java.util.concurrent.LinkedBlockingQueue}
 *
 * @param <E>
 */
public class ConfigurableQueue<E> extends AbstractQueue<E>
        implements Configurable, Lockable, BlockingQueue<E>, Serializable {
    ...
    public void setTakeTime(int min, int max, TimeUnit unit) {
        this.takeTimeRange.set(new Range(unit.toMillis(min), unit.toMillis(max)));
    }

    @Override
    public void setPutTime(int min, int max, TimeUnit unit) {
        this.putTimeRange.set(new Range(unit.toMillis(min), unit.toMillis(max)));
...
//     }
    ...
    public void put(E e) throws InterruptedException {
        ...
        try {
            ...
            es.submit(() -> {
                ...
                try {
                    Thread.sleep(putTimeRange.get().rand());
    ...
    public E take() throws InterruptedException {
        ...
        try {
            ...
            es.submit(() -> {
                ...
                try {
                    Thread.sleep(this.takeTimeRange.get().rand());
                } catch (InterruptedException es) {
    ...
    static final class LBQSpliterator<E> implements Spliterator<E> {
        ...
        final ConfigurableQueue<E> queue;
        Node<E> current;    // current node; null until initialized
        int batch;          // batch size for splits
        boolean exhausted;  // true when no more nodes
        long est;           // size estimate

        LBQSpliterator(ConfigurableQueue<E> queue) {
            this.queue = queue;
            this.est = queue.size();
        }
        ...
        public Spliterator<E> trySplit() {
            Node<E> h;
            final ConfigurableQueue<E> q = this.queue;
            int b = batch;
